...
public func evalCubicSplineDerivative(
...
/// Vectorized 1-D interpolation for an array of query points.
///
/// Evaluates the interpolant at each point in `xNew` and returns the
/// corresponding array of interpolated values. This is equivalent to
/// calling `interp1d` for each element but avoids redundant coefficient
/// setup for cubic interpolation.
///
/// - Parameters:
///   - x: Known data x values (must be strictly increasing)
///   - y: Known data y values (same length as x)
///   - xNew: Array of query points
///   - kind: Interpolation method
///   - fillValue: Value to return for out-of-bounds (if boundsError is false)
///   - boundsError: Whether to return NaN for out-of-bounds
///   - coeffs: Precomputed spline coefficients (for cubic)

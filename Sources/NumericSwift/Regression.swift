...
public func ols(_ y: [Double], _ X: [[Double]], weights: [Double]? = nil) -> OLSResult? {
  ...
  // Effective weights: use provided weights or uniform
  let w: [Double] = weights ?? [Double](repeating: 1.0, count: n)
  ...
  // Compute (X'WX)^-1 for standard errors (reduces to X'X when w=1)
  var XtX = [Double](repeating: 0.0, count: k * k)
  for i in 0..<k {
    for j in 0..<k {
      var sum = 0.0
      for obs in 0..<n {
        sum += X[obs][i] * w[obs] * X[obs][j]
      }
      XtX[j * k + i] = sum
    }
  }
  ...
  // Hat matrix diagonal: h_ii = w_i * x_i' (X'WX)^{-1} x_i
  ...
  if k > 0 {
    ...
    dsyev_(
      UnsafeMutablePointer(mutating: ("V" as NSString).utf8String),
      UnsafeMutablePointer(mutating: ("U" as NSString).utf8String),
  ...
  if infoInv == 0 && n > k {
    func computeHCStdErrors(omega: [Double]) -> [Double] {
      // Meat of sandwich: X'W diag(omega) W X
      var XtOmegaX = [Double](repeating: 0.0, count: k * k)
      for i in 0..<k {
        for j in 0..<k {
          var sum = 0.0
          for obs in 0..<n {
            sum += X[obs][i] * w[obs] * omega[obs] * w[obs] * X[obs][j]
          }
          XtOmegaX[j * k + i] = sum
        }
      }
...
private func glmLogLikelihood(y: [Double], mu: [Double], family: GLMFamily) -> Double {
  ...
  case .gaussian:
    ...
    llf =
      -Double(n) / 2.0 * Darwin.log(2.0 * .pi) - Double(n) / 2.0 * Darwin.log(ssr / Double(n))
      - Double(n) / 2.0
  case .binomial:
    for i in 0..<n {
...
private func computeGLMStandardErrors(X: [[Double]], mu: [Double], family: GLMFamily, link: GLMLink)
  -> [Double]
{
  let n = X.count
  let k = X.first?.count ?? 0
...
public func arima(_ y: [Double], p: Int, d: Int, q: Int, maxiter: Int = 100, tol: Double = 1e-8)
  -> ARIMAResult?
{

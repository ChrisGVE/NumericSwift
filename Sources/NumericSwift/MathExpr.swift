...
public struct MathExpr {
  ...
  public static func tokenize(_ expression: String) throws -> [MathExprToken] {
    ...
    while index < expression.endIndex {
      ...
      // Unknown character
      throw MathExprError.unexpectedCharacter(
        char, at: expression.distance(from: expression.startIndex, to: index))
    }
  ...
  private static func parseNumber(_ expression: String, startingAt start: String.Index) throws -> (
    Double, String.Index
  ) {
    var index = start
    var numberString = ""
  ...
  private static func parseIdentifier(_ expression: String, startingAt start: String.Index) -> (
    String, String.Index
  ) {
    var index = start
    var identifier = ""
  ...
  private static func popOperator(_ operators: inout [MathExprToken], _ output: inout [MathExprAST])
    throws
  {
  ...
  public static func evaluate(_ ast: MathExprAST, variables: [String: Double] = [:]) throws
    -> Double
  {
    switch ast {
    case .number(let n):
  ...
  private static func evaluateFunction(_ name: String, args: [Double]) throws -> Double {
    ...
    case "sin":
      guard args.count == 1 else {
        throw MathExprError.invalidArguments("\(name) requires 1 argument")
      }
      return sin(args[0])
    case "cos":
      guard args.count == 1 else {
    ...
    case "atan2":
      guard args.count == 2 else {
        throw MathExprError.invalidArguments("\(name) requires 2 arguments")
      }
    ...
    case "min":
      guard args.count >= 2 else {
        throw MathExprError.invalidArguments("\(name) requires at least 2 arguments")
      }
      return args.min()!
    case "max":
      guard args.count >= 2 else {
    ...
    case "clamp":
      guard args.count == 3 else {
        throw MathExprError.invalidArguments("\(name) requires 3 arguments")
      }
  ...
  public static func substitute(_ ast: MathExprAST, with substitutions: [String: MathExprAST])
    -> MathExprAST
  {
    switch ast {
  ...
  ///
  /// Supports the same operators and a subset of functions as the real
  /// evaluator, plus imaginary number tokens.
  public static func evaluateComplex(
    ...
    case .variable(let name):
      if let cval = complexVariables[name] {
        return cval
      }
      if let rval = variables[name] {
        return Complex(rval)
      }
      throw MathExprError.undefinedVariable(name)
    case .unary(let op, let operand):
      let val = try evaluateComplex(
        operand, variables: variables, complexVariables: complexVariables)
      switch op {
      case "-": return Complex(re: -val.re, im: -val.im)
      default: throw MathExprError.invalidArguments("unknown unary operator: \(op)")
      }
    case .binary(let op, let left, let right):
      ...
      case "^":
        ...
        return (r * l.log).exp
      default: throw MathExprError.invalidArguments("unknown operator: \(op)")
      }
    case .call(let name, let args):
      let argValues = try args.map {
        try evaluateComplex($0, variables: variables, complexVariables: complexVariables)
      }
      return try evaluateComplexFunction(name, args: argValues)
    }
  }

  private static func evaluateComplexFunction(_ name: String, args: [Complex]) throws -> Complex {
    guard args.count == 1 else {
      throw MathExprError.invalidArguments("\(name) requires 1 argument for complex evaluation")
    }
    let z = args[0]
    switch name {
    ...
    default:
      if z.im == 0 {
        let realResult = try evaluateFunction(name, args: [z.re])
        return Complex(realResult)
      }
      throw MathExprError.invalidArguments("function '\(name)' not supported for complex arguments")
    }
  }
}
